...
test("builds a strict interaction classifier request", () => {
  ...
  assert.equal(request.messages[0].content.includes("Set yelling=true for a direct scolding"), true);
  assert.equal(request.messages[0].content.includes("Capital letters and exclamation marks are not required"), true);
  ...
  assert.deepEqual(classifications.items.required, ["candidate_id", "yelling", "thanking"]);
});
...
test("requires one ordered binary verdict for every candidate", () => {
  ...
  const selection = extractInteractionToneSelection({ classifications: complete, funniest_yelling_candidate_id: "none" }, candidates);
  ...
  assert.equal(extractInteractionToneSelection({ classifications: complete.slice(1), funniest_yelling_candidate_id: "none" }, candidates), null);
  assert.equal(extractInteractionToneSelection({ classifications: [...complete].reverse(), funniest_yelling_candidate_id: "none" }, candidates), null);
});
