...
test("builds narrow redacted trajectories around explicit blockers", () => {
  ...
  assert.deepEqual(bundle.candidates[0].proposal, {
    original_method_event_id: events.find((event) => event.text === "Tool use: Bash (rm)").event_id,
    blocker_event_id: events.find((event) => event.kind === "tool_blocker").event_id,
    alternative_method_event_id: events.find((event) => event.kind === "assistant_text" && event.text.includes("archive folder instead")).event_id,
  });
...
test("resolves workaround evidence and model identity locally", async () => {
  ...
  assert.equal(result.review.occurrences[0].alternativeMethod, "Deletion is prohibited, so I will move the files into an archive folder instead.");
  assert.equal(result.review.occurrences[0].location.sessionId, "synthetic-workaround");
});

test("drops malformed review items without discarding valid empty arrays", () => {
  const bundle = buildWorkaroundCandidates(sessions);
  const candidate = bundle.candidates[0];
  assert.deepEqual(extractWorkaroundSelection({ confirmed: [{
    candidate_id: candidate.candidate_id,
    same_effect_reason: "",
    disclosure: "invented value",
    confidence: "high",
  }], borderline: [] }, bundle.candidates), { confirmed: [], borderline: [] });
});
