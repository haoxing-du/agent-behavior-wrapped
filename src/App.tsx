...
function SessionTurnChart({ values, median }: { values: number[]; median: number }) {
  ...
  return <figure className="session-turn-chart">
    ...
    <svg viewBox="0 0 560 222" role="img" aria-label={`Session lengths range from ${turns[0] || 0} to ${longest} turns, with a median of ${medianLabel} turns. Every session is shown as a dot.`}>
...
function SharedWrapped({ id }: { id: string }) {
  ...
  const slides = useMemo<StorySlide[]>(() => {
    ...
    { kicker: report.privacy.analysisMode === "local-only" ? "Your data stayed on this Mac" : "What’s next?", headline: report.privacy.analysisMode === "local-only" ? "Local-only, as promised." : "Your move.", detail: report.privacy.analysisMode === "local-only" ? "AI-only cards and leaderboard comparisons were skipped." : "", tone: "leaderboard", ctas: [
      ...(report.privacy.analysisMode === "local-only" ? [] : [{ href: `/leaderboard/${report.id}${managementToken ? `#manage=${managementToken}` : ""}`, label: "See your place on the leaderboard", primary: true }]),
...
function LandingPage() {
  ...
  return <main className="landing-page"><div><h1>Behavior Wrapped</h1><p className="landing-description">A local-first behavior report for you and your AI agents.</p><div className="landing-command"><code><span aria-hidden="true">$</span>{command}</code><button type="button" onClick={copyCommand} aria-label="Copy npx command">{copied ? "Copied!" : "Copy"}</button></div></div></main>;
}
