...
function SharedWrapped({ id }: { id: string }) {
  ...
  const slides = useMemo<StorySlide[]>(() => {
    ...
    const sortedStockPhrases = stockPhrases?.slice().sort((left, right) => right.count - left.count || left.phrase.localeCompare(right.phrase));
    ...
    ...(sortedStockPhrases ? [{ kicker: "Models love these phrases", headline: "Your agents were no exception.", detail: "Here’s how many times yours used each one.", tone: "stock", rows: sortedStockPhrases.map((item) => ({ label: `“${item.phrase}”`, value: item.count.toLocaleString() })) }] : []),
