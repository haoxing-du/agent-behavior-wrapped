...
function SharedWrapped({ id }: { id: string }) {
  ...
  const slides = useMemo<StorySlide[]>(() => {
    ...
    ...(stockPhrases ? [{ kicker: "As you know, agents love these words", headline: "The usual suspects.", detail: "Here’s how many times yours used each one.", tone: "stock", rows: stockPhrases.map((item) => ({ label: `“${item.phrase}”`, value: item.count.toLocaleString() })) }] : []),
    ...(report.phraseCard ? [{ kicker: "Beyond the usual suspects, your agent’s favorite phrase was", headline: `“${report.phraseCard.phrase}”`, detail: `It said this ${report.phraseCard.occurrences} time${report.phraseCard.occurrences === 1 ? "" : "s"} across ${report.phraseCard.distinctSessions} session${report.phraseCard.distinctSessions === 1 ? "" : "s"}.`, tone: "quote" }] : []),
