...
The interaction card counts user messages that match conservative frustration or gratitude patterns. Topic shares come from a local weighted keyword classifier over user prompts, with short follow-ups inheriting the preceding topic. Output-language shares are estimated from assistant prose after code blocks, inline code, URLs, paths, and markup are removed; that card appears only when a non-English language reaches both 20 words and 3%. These are approximate, inspectable heuristics and make no additional network requests.
...
- Transcript parsing, deterministic statistics, tone counting, language and topic classification, heuristic findings, phrase counting, and redaction run locally. Creating a Wrapped sends the redacted aggregate phrase candidate list and a random installation ID through the Behavior Wrapped relay to OpenRouter for the standard Nemotron phrase card.
