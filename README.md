...
Every Wrapped uses the free Nemotron 3 Ultra model through the hosted Behavior Wrapped relay and OpenRouter for favorite-phrase selection and interaction-tone classification. Only redacted candidate excerpts, counts, and a random installation ID are sent; full transcripts, tool output, code, paths, and detected secrets are not included. The relay accepts only fixed schemas, uses a fixed free model, and rate-limits clients before attaching its server-side OpenRouter credential.
...
- Transcript parsing, deterministic statistics, language and topic classification, heuristic findings, phrase counting, candidate selection, and redaction run locally. After CLI consent, creating a Wrapped sends only redacted favorite-phrase and interaction-tone candidates plus counts and a random installation ID through the Behavior Wrapped relay to OpenRouter.
