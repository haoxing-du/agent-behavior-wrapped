...
The interaction card counts user messages that match conservative frustration or gratitude patterns. Nemotron chooses the funniest frustration quote and most wholesome thank-you from narrow candidate lists after code, paths, URLs, likely secrets, and PII are removed; candidates containing redaction placeholders are excluded entirely. Topic shares come from a local weighted keyword classifier over user prompts, with short follow-ups inheriting the preceding topic. Output-language shares are estimated from assistant prose after code blocks, inline code, URLs, paths, and markup are removed; that card appears only when a non-English language reaches both 20 words and 3%.
